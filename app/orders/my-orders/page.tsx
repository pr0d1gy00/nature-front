...
export default function Page() {
    const { allOrders, setShowModal } = useOrder();
    ...
    const getStatusColor = (status: string) => {
        switch (status.toLowerCase()) {
            ...
            case 'pagado': return 'bg-blue-200 text-blue-800';
    ...
    return (
        <Layout>
            <motion.section className="max-md:w-[100%] max-md:items-start flex flex-col items-center justify-start min-h-screen h-auto w-full bg-gradient-to-br from-[#fdfee7] via-[#fdfee7] to-[#32384549] py-8">
                <div className="w-[85%] max-w-[90%] mx-auto">
                    ...
                    {/* Maqueta de la Sección de Filtros */}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    ...
                                    name="search"
                                    id="search"
                                    placeholder="Nombre, email o ID de orden..."
                    ...
                    {!allOrders || !allOrders.orders || allOrders.orders.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                            {allOrders.orders.map((order) => (
                                <div key={order.id} className="bg-white rounded-lg shadow-lg overflow-hidden transform hover:-translate-y-1 transition-transform duration-300 flex flex-col">
                                    <div className="p-6 flex-grow">
                                        ...
                                        <div className="mb-4">
                                            <h3 className="text-md font-semibold text-gray-700 mb-2">Dirección de Envío:</h3>
                                            ...
                                            <ul className="space-y-3 max-h-40 overflow-y-auto pr-2">
                                                {order.order_products.map((item) => (
                                                    <li key={item.id} className="flex justify-between items-center text-sm">
                                                        <span className="text-gray-800 truncate pr-2">{item.product.name} (x{item.quantity})</span>
                                                        <span className="font-medium text-gray-600 whitespace-nowrap">${(parseFloat(item.price) * item.quantity).toFixed(2)}</span>
                                                    </li>
                                                ))}
                                            </ul>
                                        </div>
                                    ...
                                    <div className="bg-gray-50 px-6 py-4 flex justify-between items-center border-t">
                                        ...
                                        <span className="text-2xl font-bold text-green-600">${parseFloat(order.total).toFixed(2)}</span>
                                    </div>
                                    <button onClick={() => setShowModal(true)} className="bg-green-500 text-white px-4 font-bold py-2 m-4 rounded hover:bg-green-600 transition-colors duration-300">
                                        Ver Pago
                                    </button>
                                </div>
                            ))}
