...
export default function Page() {
	const { inputs,handleCodeChange,codeRefs } = useResetPassword();
	return (
				...
				<div className='max-md:w-[90%] max-md:items-start flex flex-col items-start w-[70%]'>
					...
					<form action="" className='max-md:w-[90%] w-[70%] flex flex-col items-start mt-6'>
						{inputs.map((input, index) => (
							<Input key={index} id={input.id} name={input.id} type={input.type} placeholder={input.placeholder}/>
						))}
						...
						<div className='flex items-center justify-between w-full mb-4'>
							...
							<Link href="/auth/reset-password" className='text-md font-bold text-[#35384b]'>Vuelve al login</Link>
						</div>
						<Button title="Enviar" />
					</form>
			</div>
